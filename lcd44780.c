...
#define SET_RS 	SET(LCD_RS)			// stan wysoki na linii RS
#define CLR_RS 	CLR(LCD_RS)		// stan niski na linii RS

#define SET_RW 	SET(LCD_RW)		// stan wysoki na RW - odczyt z LCD
#define CLR_RW 	CLR(LCD_RW)		// stan niski na RW - zapis do LCD

#define SET_E 	SET(LCD_E)			// stan wysoki na linii E
#define CLR_E 	CLR(LCD_E)			// stan niski na linii E
...
{
	if (data&(1<<0)) SET(LCD_D4); else CLR(LCD_D4);
	if (data&(1<<1)) SET(LCD_D5); else CLR(LCD_D5);
	if (data&(1<<2)) SET(LCD_D6); else CLR(LCD_D6);
	if (data&(1<<3)) SET(LCD_D7); else CLR(LCD_D7);
}
#if USE_RW == 1
//----------------------------------------------------------------------------------------
//
...
static inline uint8_t lcd_readHalf(void)
{
	...
	if(INPUT(LCD_D4)) result |= (1<<0);
	if(INPUT(LCD_D4)) result |= (1<<1);
	if(INPUT(LCD_D4)) result |= (1<<2);
	if(INPUT(LCD_D4)) result |= (1<<3);
